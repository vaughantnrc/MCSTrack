...
class PoseSolverParameters(BaseModel):
    ...
    MAXIMUM_RAY_COUNT_FOR_INTERSECTION: int = Field(default=2)
    POSE_MULTI_CAMERA_LIMIT_RAY_AGE_SECONDS: float = Field(default=0.1)
    POSE_SINGLE_CAMERA_EXTRAPOLATION_MINIMUM_SURFACE_NORMAL_ANGLE_DEGREES: float = Field(default=15.0)
    POSE_SINGLE_CAMERA_EXTRAPOLATION_LIMIT_RAY_AGE_SECONDS: float = Field(default=1.0)
    POSE_SINGLE_CAMERA_EXTRAPOLATION_MAXIMUM_ORDER: int = Field(default=0)
    POSE_SINGLE_CAMERA_EXTRAPOLATION_LIMIT_ANGLE_DEGREES: float = Field(default=15.0)
    POSE_SINGLE_CAMERA_EXTRAPOLATION_LIMIT_DISTANCE: float = Field(default=15.0, description="millimeters")
    POSE_SINGLE_CAMERA_NEAREST_LIMIT_RAY_AGE_SECONDS: float = Field(default=0.8)
    POSE_SINGLE_CAMERA_NEAREST_LIMIT_ANGLE_DEGREES: float = Field(default=15.0)
    POSE_SINGLE_CAMERA_NEAREST_LIMIT_DISTANCE: float = Field(default=15.0)
    POSE_SINGLE_CAMERA_REPROJECTION_ERROR_FACTOR_BETA_OVER_ALPHA: float = Field(default=1.0)
    POSE_SINGLE_CAMERA_DEPTH_LIMIT_AGE_SECONDS: float = Field(default=0.4)
    # TODO: Is this next one detector-specific?
    POSE_SINGLE_CAMERA_DEPTH_CORRECTION: float = Field(default=-7.5, description="millimeters, observed tendency to overestimate depth.")
    POSE_DETECTOR_DENOISE_LIMIT_AGE_SECONDS: float = Field(default=1.0)
    INTERSECTION_MAXIMUM_DISTANCE: float = Field(default=10.0, description="millimeters")
    ...
    DENOISE_OUTLIER_DISTANCE_MILLIMETERS: float = Field(default=10.0)
    DENOISE_OUTLIER_ANGLE_DEGREES: float = Field(default=5.0)
    ...
    ARUCO_MARKER_DICTIONARY_ENUM: int = Field(default=cv2.aruco.DICT_4X4_100)
    ARUCO_POSE_ESTIMATOR_METHOD: int = Field(default=cv2.SOLVEPNP_ITERATIVE)
